...
static enum IMGSENSOR_RETURN gpio_release(void *pinstance)
{
	...
	j < IMGSENSOR_SENSOR_IDX_MAIN2;
	j++) {
		for (i = GPIO_CTRL_STATE_RST_H;
			i < GPIO_CTRL_STATE_LDO_VCAMA_H;
			i += 2) {
			lookup_names =
			...
			if (lookup_names != NULL &&
				...
				pr_info(
                                    "%s : pinctrl gpio Success release, PinIdx %d name %s\n",
                                    __func__,
                                    i,
                                    lookup_names);
			}
...
static enum IMGSENSOR_RETURN gpio_init(void *pinstance)
{
	...
	j < IMGSENSOR_SENSOR_IDX_MAIN2;
	j++) {
		for (i = GPIO_CTRL_STATE_RST_H; i <  GPIO_CTRL_STATE_LDO_VCAMA_H; i++) {
			lookup_names =
				gpio_pinctrl_list_cam[i].ppinctrl_lookup_names;
			if (lookup_names) {
