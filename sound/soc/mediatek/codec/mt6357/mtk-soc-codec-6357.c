...
static void Ext_Speaker_Call_Change(bool enable)
{
	pr_debug("%s(), enable %d\n", __func__, enable);
#define SPK_WARM_UP_TIME        (25)	/* unit is ms */
	if (enable) {
...
static int Ext_Speaker_Call_Get(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	/* pr_debug("%s()\n", __func__); */
	ucontrol->value.integer.value[0] =
		mCodec_data->mAudio_Ana_DevicePower
			[AUDIO_ANALOG_DEVICE_OUT_EXTSPKAMP_CALL];
	return 0;
}
static int Ext_Speaker_Call_Set(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	pr_debug("%s() gain = %ld\n ", __func__,
		 ucontrol->value.integer.value[0]);
	if (ucontrol->value.integer.value[0]) {
		Ext_Speaker_Call_Change(true);
		mCodec_data->mAudio_Ana_DevicePower
			[AUDIO_ANALOG_DEVICE_OUT_EXTSPKAMP_CALL] =
		    ucontrol->value.integer.value[0];
	} else {
...
static const struct snd_kcontrol_new mt6357_snd_controls[] = {
	...
	SOC_ENUM_EXT("Ext_Speaker_Call_Switch", Audio_DL_Enum[17],
		     Ext_Speaker_Call_Get,
		     Ext_Speaker_Call_Set),
